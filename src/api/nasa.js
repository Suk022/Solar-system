...
export const fetchPlanetData = async (planetName) => {
  try {
    ...
    const imagesResponse = await fetch(
      `${API_URL}/search?q=${searchQuery}&media_type=image`,
    );
    ...
    let fact = null;
    const itemsWithDescriptions = imagesData.collection.items.filter(
      (item) =>
    ...
    if (itemsWithDescriptions.length > 0) {
      const randomItem =
        itemsWithDescriptions[
          Math.floor(Math.random() * itemsWithDescriptions.length)
        ];
      fact = {
